...
func (r *EventReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	defer func() {
		...
		// we should move the corresponding pipelinerun to failed, otherwise
		// it will remains in running state, waiting for the secret to be ready
		// until timeout.
		if pod.Name != "" && errMessage != "" {
			...
			if err := r.Get(ctx, client.ObjectKey{Namespace: pod.Namespace, Name: plrName}, &plr); err != nil {
				if apierrors.IsNotFound(err) {
					// The PipelineRun is gone, so we can't update it.
					return
				}
				log.Error(err, "failed to get the corresponding pipelinerun in defer", "pod", pod.Name)
